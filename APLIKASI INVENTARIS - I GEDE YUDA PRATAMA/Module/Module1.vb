...
Module Module1
    ...
    'belum dipakai
    Public Sub ExecuteRibbonControlStyle(ByVal RibbonControl As DevExpress.XtraBars.Ribbon.RibbonControl)
        RibbonControl.RibbonStyle = My.Settings.RibbonControlStyle
    End Sub
    ...
    Public Sub ExecuteComboBoxListWithConcat(ByVal TableName As String, ByVal ConcatGroupFields As String, ByVal FieldName As String, ByVal ComboBoxEditName As DevExpress.XtraEditors.ComboBoxEdit, Optional ByVal Criteria As String = Nothing, Optional ByVal ListAdd As String = Nothing)
        ComboBoxEditName.Properties.Items.Clear()
        If ListAdd <> Nothing Then
            Dim List = ListAdd.Split(",")
            For Each i In List
        ...
        Dim QueryString As String
        If Criteria <> Nothing Then
            QueryString = "Select distinct(" & ConcatGroupFields & ") as " & FieldName & " from " & TableName & " where " & Criteria
        Else
            QueryString = "Select distinct(" & ConcatGroupFields & ") as " & FieldName & " from " & TableName
        End If
        Dim Connection As New MySqlConnection(My.Settings.ConString)
        Dim DataAdapter As New MySqlDataAdapter(QueryString, Connection)
        Dim DataTable As New DataTable
